<script lang="ts">
  ...
  let mappings = $derived.by(() => {
    ...
    // Sort targets by name (or should we trust selection order? 
    // Usually selection order in TreeView might be arbitrary if user click-clicked.
    // Let's sort targets by name to be safe for sequences.)
  ...
  function handleDrop(e: DragEvent) {
    ...
    if (e.dataTransfer?.files) {
      sourceFiles = Array.from(e.dataTransfer.files);
    }
  }
...
<div class="sequence-replacer">
  <div class="header">
    ...
    <p>Map local files to selected firmware images.</p>
  </div>
    ...
    <div class="column sources"
      ...
      <div class="list drop-zone">
        {#if sourceFiles.length === 0}
            <div class="empty-msg">Drag & Drop files here</div>
        {:else}
            {#each mappings as map}
